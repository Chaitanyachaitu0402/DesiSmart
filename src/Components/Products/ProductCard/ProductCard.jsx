...
const ProductCard = ({ product }) => {
  const { Name, SalePrice, Supplier, product_image } = product;
  ...
  const handleAddToCartBtn = () => {
    ...
    const targetedProduct = {
      ...
      quantity: existingProduct
        ...
        : quantity,
      total: (
        (existingProduct ? existingProduct.quantity + quantity : quantity) *
        SalePrice
      ).toFixed(2),
    };
    ...
    const updatedCartItems = existingProduct
      ? cartItems.map((item) =>
          item.id === product.Id ? targetedProduct : item
        )
      ...
      <Fade in={true}>
        <Card
          sx={{
            maxWidth: 295,
            mx: "auto",
            boxShadow: "0 9px 5px -0px rgb(0 0 0 / 0.4)",
          }}
        >
          <div className="p-1.5">
